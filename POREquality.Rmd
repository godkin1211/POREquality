...
parser <- add_option(parser,
                     opt_str=c("-b", "--barcode"),
                     type="character",
                     dest='barcoding.summary',
                     help="Input path to a Nanopore barcoding-summary file"
                     )
parser <- add_option(parser,
                     opt_str=c("-o", "--output"),
                     type="character",
...
#parser <- add_option(parser,
#                     opt_str=c("-d", "--demultiplex"),
#                     type="character",
#                     dest='barcoding.summary',
#                     help="Input path to the guppy_barcoder output")
options=parse_args(parser)
input.file=options$input.file
barcoding.summary=options$barcoding.summary
...
run.length = (max(as.numeric(seq_dt$start_time))) %/% 60
```
### Total yield (gbp)
...
barcodeTable=fread(barcoding.summary, header=TRUE, sep="\t")
barcode=barcodeTable %>% group_by(barcode_arrangement) %>% dplyr::summarise(freq=n())
colnames(barcode)[1]="x"
